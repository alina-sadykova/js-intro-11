...
// OPTION 2
const olderThanThirty = users.reduce((acc, curr) => {
  if (curr.age > 30) acc++;
  return acc;
}, 0);
...
console.log("\n--------TASK2--------\n");
const livesInChicago = users.filter(
  (user) => user.address.city === "Chicago"
).length;
console.log(livesInChicago);

console.log("\n--------TASK3--------\n");
const livesInIl = users.filter((user) => user.address.state === "IL").length;
console.log(livesInIl);

console.log("\n--------TASK4--------\n");
const findGmailDomain = users.filter(
  (user) => user.email.split("@")[1].slice(0, -4) === "gmail"
).length;
console.log(findGmailDomain);

console.log("\n--------TASK5--------\n");
const usersYoungerThan30 = users.reduce((names, user) => {
...
const _olderThan30 = users.reduce(
  (counter, user) => (user.age > 30 ? counter + 1 : counter),
  0
);
console.log(_olderThan30);

//•	Count how many users live in Chicago     			-> 2
const liveInChicago = users.reduce(
  (counter, user) => (user.address.city === "Chicago" ? counter + 1 : counter),
  0
);
console.log(liveInChicago);

//•	Count how many users live in IL     -> 3
const liveInIllinois = users.reduce(
  (counter, user) => (user.address.state === "IL" ? counter + 1 : counter),
  0
);
console.log(liveInIllinois);

//•	Count how many users’ emails has Gmail domain  -> 2
const gmailDomain = users.reduce(
  (count, user) => (user.email.includes("gmail") ? count + 1 : count),
  0
);
console.log(gmailDomain);

//•	Find and store all the users name younger than 35    		-> [ 'Tech', 'Jane', 'Alex' ]
const youngerThan35 = users.reduce(
  (nameArr, user) => (user.age < 35 ? nameArr.concat(user.firstName) : nameArr),
  []
);
...
const averageOfAges =
  users.reduce((count, user) => count + user.age, 0) / users.length;
...
// Find the most expensive product   -> MacBook Pro 16-inch
const mostExpensive = appleStore.reduce(
  (max, curr) => (max.price > curr.price ? max : curr),
  {}
);
console.log(mostExpensive.productName);
// Find the least expensive product   -> AirPods Pro
const leastExpensive = appleStore.reduce(
  (min, curr) => (min.price < curr.price ? min : curr),
  {}
);
console.log(leastExpensive.productName);
// Find the product with the biggest quantity   -> AirPods Pro
const mostQuantity = appleStore.reduce(
  (mostCounted, curr) =>
    mostCounted.quantity > curr.quantity ? mostCounted : curr,
  {}
);
console.log(mostQuantity.productName);
// Find the product with the smallest quantity  -> iPhone 14 Pro
const leastQuantity = appleStore.reduce(
  (leastCounted, curr) =>
    leastCounted.quantity < curr.quantity ? leastCounted : curr,
  {}
);
...
// Find the brands of all the products in the cart  -> [ 'Dell', 'Apple', 'Sony' ]
const productBrands = shoppingCart.items.map(
  (item) => item.specifications.brand
);
console.log(productBrands); // [ 'Dell', 'Apple', 'Sony' ]
// Find all the items in the cart with their quantity -> [ 'Laptop, 1', 'Smartphone 2', 'Headphones, 3' ]
const itemsWithQuantities = shoppingCart.items.map(
